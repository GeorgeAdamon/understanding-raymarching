...
namespace RayMarching.Runtime.CPU
{
    public class RayMarcher:MarcherBase
    {
        ...
        protected override void Execute()
        {
            ...
            else
            {
                for (var i = 0; i < rayEntryPoints.Length; i++)
                {
                    ...
                    // Create Ray
                    var ray       = new Ray(rayEntryPoints[i], rayExitPoints[i] - rayEntryPoints[i]);
                    var cnt       = 0;
                    var dist      = 0f;
                    var rayLength = Vector3.Distance(rayExitPoints[i], rayEntryPoints[i]);
                    
                    for (var j = 0; j < maxStepsPerRay; j++)
                    {
                        if (dist > rayLength)
                            break;

                        interSectionPoints[i * maxStepsPerRay + cnt] =  ray.GetPoint(dist);
                        dist                                      += fixedStep;
                        cnt++;
                    }
            ...
            {
                if (visMesh == null || visMesh.vertexCount != interSectionPoints.Length)
                {
                    if (visMesh == null)
                        visMesh = new Mesh();
        ...
        [BurstCompile]
        private struct RayMarchingJob :IJobParallelFor
        {
            [ReadOnly]
            public NativeArray<float3> entryPoints;
            [ReadOnly]
            public NativeArray<float3> exitPoints;
            [ReadOnly]
            ...
            [WriteOnly]
            public NativeArray<int>    intersectionsPerRay;
            ...
            public void Execute(int i)
            {
                ...
                for (var j = 0; j < maxSteps; j++)
                {
                    if (plane.Raycast(ray, out var enter))
                    {
                        ...
                        {
                            interSectionPoints[startIndex + cnt] = ray.GetPoint(enter);
                            cnt++;
                        }
                    }
                    plane.distance += step;
